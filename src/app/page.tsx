...
export default function Home() {
  const { profile, user, loading, connectedSoldier } = useAuth()
  ...
  // For non-soldier with connection, use connected soldier's data
  const effectiveBranch = (!isSoldier && connectedSoldier?.branch) 
    ? (connectedSoldier.branch as Branch) 
    : ((profile?.branch as Branch) || 'army')
  const effectiveEnlistDate = (!isSoldier && connectedSoldier?.enlist_date) 
    ? connectedSoldier.enlist_date as string
    : (profile?.enlist_date || '')
  const effectiveRank = effectiveEnlistDate 
    ? calcAutoRank(effectiveEnlistDate, effectiveBranch) 
    : ((profile?.rank_level as RankLevel) || 1)

  const currentBranch = BRANCHES.find(b => b.value === effectiveBranch)!
  const accentColor = { army: '#2d5016', navy: '#1a365d', airforce: '#4a1d96', marines: '#991b1b', katusa: '#967117' }[effectiveBranch]
    ...
    <div style={{ maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minHeight: '48px' }}>
          <div style={{
            ...
            {avatarUrl ? (
              ...
              <RankIcon level={effectiveRank} branch={effectiveBranch} size={28} />
            )}
          </div>
          <div>
            <div style={{ fontSize: '17px', fontWeight: 800, whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
              {displayName && dday ? (
                <>
                  <span>{isSoldier 
                    ? `${currentBranch.label} ${RANK_LABELS[effectiveRank]} ${displayName}`
                    : `${displayName}`
                  }</span>
                  {!isSoldier && connectedSoldier && (
                    <span style={{ fontSize: '11px', opacity: 0.8 }}>({connectedSoldier.nickname || connectedSoldier.display_name} 용사)</span>
                  )}
            ...
            {(!name || !dday) && (
              <div style={{ fontSize: '12px', opacity: 0.8, marginTop: '2px' }}>
                {!isSoldier && !connectedSoldier ? '마이페이지에서 용사를 연동해보세요!' : '군 생활의 든든한 동반자'}
              </div>
            )}
          </div>
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
          ...
          <span style={{ fontSize: '11px', color: '#6b7280' }}>{currentBranch.label} {SERVICE_MONTHS[effectiveBranch]}개월</span>
        </div>
          ...
          <div style={{
            ...
            {isSoldier ? '마이페이지에서 정보를 입력해주세요.' : '용사 연동 후 복무기간이 표시됩니다.'}
          </div>
        )}
